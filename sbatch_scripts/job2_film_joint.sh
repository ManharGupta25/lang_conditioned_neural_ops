#!/bin/bash
#SBATCH --job-name=film_joint
#SBATCH --partition=goodarzilab_gpu_priority,gpu_batch_high_mem,gpu_batch
#SBATCH --gres=gpu:1
#SBATCH --mem=100G
#SBATCH --cpus-per-task=16
#SBATCH --output=slurm_logs/job2_film_joint_%j.out
#SBATCH --error=slurm_logs/job2_film_joint_%j.err
#SBATCH --time=12:00:00

# ── Job 2: film, joint fine-tuning — 3 missing PDEs ──
# Missing: phy_adv_diff, phy_ks, phy_burgers
...
python train.py \
    ...
    --freeze-trunk false \
    --scenarios phy_adv_diff phy_ks phy_burgers
